...
# disasm mode
CS_MODE_LITTLE_ENDIAN = 0      # default mode
CS_MODE_SYNTAX_INTEL = 0       # default X86 asm syntax (applicable for CS_ARCH_X86 only)
CS_MODE_ARM = 0
CS_MODE_16 = (1 << 1)
CS_MODE_32 = (1 << 2)
CS_MODE_64 = (1 << 3)
CS_MODE_THUMB = (1 << 4)
CS_MODE_SYNTAX_ATT = (1 << 30)   # X86 ATT asm syntax (applicable for CS_ARCH_X86 only)
...
_all_libs = ['capstone.dll', 'libcapstone.so', 'capstone.dylib']
_found = False
...
_setup_prototype(_cs, "cs_open", ctypes.c_bool, ctypes.c_uint, ctypes.c_uint, ctypes.POINTER(ctypes.c_uint64))
...
_setup_prototype(_cs, "cs_close", ctypes.c_bool, ctypes.POINTER(ctypes.c_uint64))
...
class cs_insn(object):
    def __init__(self, csh, all_info, arch):
        ...
        if arch == CS_ARCH_ARM:
            (self.cc, self.update_flags, self.writeback, self.op_info) = \
            ...
            (self.prefix, self.segment, self.opcode, self.op_size, self.addr_size, \
             ...
             self.sib_index, self.sib_scale, self.op_info) = x86.get_arch_info(all_info.arch.x86)
