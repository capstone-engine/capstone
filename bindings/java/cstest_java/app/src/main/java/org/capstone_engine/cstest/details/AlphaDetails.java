...
public class AlphaDetails {
    public static boolean testExpected(Capstone.CsInsn actual, Map<String, Object> expected) {
        Alpha.OpInfo alpha = (Alpha.OpInfo) actual.operands;
        if (expected.get("operands") == null) {
            return true;
        }
        
        List<Map<String, Object>> expectedOperands = (List<Map<String, Object>>) expected.get("operands");
        if (!Compare.compareUInt32(alpha.op.length, expectedOperands.size(), "operands_count")) {
            return false;
        }
        for (int i = 0; i < alpha.op.length; i++) {
            Alpha.Operand aop = alpha.op[i];
            ...
            switch (aop.type) {
                case ALPHA_OP_REG:
                    ...
                    break;
                case ALPHA_OP_IMM:
                    if (!Compare.compareInt64(aop.value.imm, Details.getLongFromMap(expected, "imm"), "imm")) {
                        return false;
                    }
                    break;
                default:
                    throw new IllegalArgumentException("Alpha operand type not handled");
            }
        }
        return true;
    }
