...
static bool read_varuint32(const uint8_t *code, size_t code_len, uint16_t *param_size, MCInst *MI)
{
	...
	if (MI->flat_insn->detail) {
		...
		MI->flat_insn->detail->wasm.operands = cs_mem_malloc(sizeof(cs_wasm_op));
		if (!(MI->flat_insn->detail->wasm.operands)) {
			return false;
		}
...
static bool read_memoryimmediate(const uint8_t *code, size_t code_len, uint16_t *param_size, MCInst *MI)
{
	...
	if (MI->flat_insn->detail) {
		...
		MI->flat_insn->detail->wasm.operands = cs_mem_malloc(sizeof(cs_wasm_op) * 2);
		if (!(MI->flat_insn->detail->wasm.operands)) {
			return false;
		}
	}
	...
	if (tmp == -1) {
		cs_mem_free(MI->flat_insn->detail->wasm.operands);
		return false;
	}
...
static bool read_brtable(const uint8_t *code, size_t code_len, uint16_t *param_size, MCInst *MI)
{
	...
	void *tmp_target = NULL, *tmp_detail = NULL;
	...
	if (code_len < tmp_len + length) {
		// if code length smaller than min length of table, return false
		return false;
	}

	cache_size = length / 4;

	MI->wasm_data.brtable.target = cs_mem_malloc(sizeof(int32_t) * cache_size);
	...
	if (MI->flat_insn->detail) {
		MI->flat_insn->detail->wasm.op_count = length + 2;
		MI->flat_insn->detail->wasm.operands = cs_mem_malloc(sizeof(cs_wasm_op) * (cache_size + 2));
		if (!MI->flat_insn->detail->wasm.operands) {
			cs_mem_free(MI->wasm_data.brtable.target);
			return false;
		}
		MI->flat_insn->detail->wasm.operands[0].type = WASM_OP_VARUINT32;
		MI->flat_insn->detail->wasm.operands[0].size = var_len;
		MI->flat_insn->detail->wasm.operands[0].varuint32= length;
	}

	// read data
	for(i = 0; i < length; i++){
		if (code_len < tmp_len || (code_len - tmp_len) < (length - cache_used)) {
			// exit when code length less than tmp length
			//      when remain code length is not enough for th param length
			cs_mem_free(MI->wasm_data.brtable.target);
			if (MI->flat_insn->detail) {
				cs_mem_free(MI->flat_insn->detail->wasm.operands);
			}
		...
		if (cache_used >= cache_size) {
			if (cache_size * 2 >= length)
				cache_size = length;
			else
				cache_size = cache_size * 2;
	
			tmp_target = cs_mem_realloc(MI->wasm_data.brtable.target, (sizeof(uint32_t)) * (cache_size));
			tmp_detail = cs_mem_realloc(MI->flat_insn->detail->wasm.operands, (sizeof(cs_wasm_op) * (cache_size + 2)));
			if (tmp_target == NULL || tmp_detail == NULL) {
				cs_mem_free(MI->wasm_data.brtable.target);
				if (MI->flat_insn->detail) {
					cs_mem_free(MI->flat_insn->detail->wasm.operands);
				}
			...
			MI->wasm_data.brtable.target = tmp_target;
			MI->flat_insn->detail->wasm.operands = tmp_detail;
		}
		...
		if (MI->flat_insn->detail) {
			MI->flat_insn->detail->wasm.operands[i + 1].type = WASM_OP_VARUINT32;
			MI->flat_insn->detail->wasm.operands[i + 1].size = var_len;
			MI->flat_insn->detail->wasm.operands[i + 1].varuint32= MI->wasm_data.brtable.target[i];
		}
	...
	if (MI->flat_insn->detail) {
		MI->flat_insn->detail->wasm.operands[length + 1].type = WASM_OP_VARUINT32;
		MI->flat_insn->detail->wasm.operands[length + 1].size = var_len;
		MI->flat_insn->detail->wasm.operands[length + 1].varuint32 = default_target;
	}

	MI->wasm_data.brtable.default_target = default_target;
	MI->wasm_data.type = WASM_OP_BRTABLE;
	*param_size = var_len + tmp_len;
		...
		MCInst *MI, uint16_t *size, uint64_t address, void *inst_info)
{
	...
	switch(opcode) {
		...
		case WASM_INS_BR_TABLE:
			...
			if (MI->flat_insn->detail) {
				MI->flat_insn->detail->wasm.op_count = MI->wasm_data.brtable.length + 2;
				MI->flat_insn->detail->groups[MI->flat_insn->detail->groups_count] = WASM_GRP_CONTROL;
				MI->flat_insn->detail->groups_count++;
			}
