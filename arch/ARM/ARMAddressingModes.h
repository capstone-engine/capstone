...
typedef enum ARM_AM_AddrOpc { ARM_AM_sub = 0, ARM_AM_add } ARM_AM_AddrOpc;
...
static inline unsigned getSORegOffset(unsigned Op) { return Op >> 3; }
...
/// the 8-bit imm value.
static inline unsigned getSOImmValImm(unsigned Imm) { return Imm & 0xFF; }

/// getSOImmValRot - Given an encoded imm field for the reg/imm form, return
/// the rotate amount.
static inline unsigned getSOImmValRot(unsigned Imm) { return (Imm >> 8) * 2; }
...
static inline unsigned getAM2IdxMode(unsigned AM2Opc) { return (AM2Opc >> 16); }
...
static inline unsigned getAM3IdxMode(unsigned AM3Opc) { return (AM3Opc >> 9); }
...
static inline uint64_t ARM_AM_decodeNEONModImm(unsigned ModImm,
                                               unsigned *EltBits) {
...
static unsigned ARM_AM_getVMOVModImmOpCmode(unsigned ModImm) {
  return (ModImm >> 8) & 0x1f;
}

static unsigned ARM_AM_getVMOVModImmVal(unsigned ModImm) {
  return ModImm & 0xff;
}
...
/// element value and the element size in bits.  (If the element size is
/// smaller than the vector, it is splatted into all the elements.)
static uint64_t ARM_AM_decodeVMOVModImm(unsigned ModImm, unsigned *EltBits) {
