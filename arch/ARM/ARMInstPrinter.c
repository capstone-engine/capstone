...
static void printMVEVectorList(MCInst *MI, unsigned NumRegs, SStream *O, unsigned OpNum);

static void printVMOVModImmOperand(MCInst *MI, unsigned OpNum, SStream *O);

static void printMveAddrModeRQOperand(MCInst *MI, unsigned OpNum, SStream *O, int shift);

static void printMandatoryRestrictedPredicateOperand(MCInst *MI, unsigned OpNum, SStream *O);

static void printMandatoryInvertedPredicateOperand(MCInst *MI, unsigned OpNum, SStream *O);
...
static void printCustomAliasOperand(const MCInst *MI,
...
static void printRegImmShift(MCInst *MI, SStream *O, ARM_AM_ShiftOpc ShOpc, unsigned ShImm) {
    ...
    if (MI->csh->detail) {
        if (MI->csh->doing_mem)
            ...
            MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count -
                                                1].shift.type = (arm_shifter) ShOpc;
    }
    ...
    if (ShOpc != ARM_AM_rrx) {
        ...
        if (MI->csh->detail) {
            if (MI->csh->doing_mem)
                MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].shift.value = translateShiftImm(
                        ShImm);
            else
                MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count -
                                                    1].shift.value = translateShiftImm(ShImm);
        }
    }
}
    ...
    switch (Opcode) {
...
#endif
                    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].type = ARM_OP_REG;
                    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].reg = MCOperand_getReg(
        ...
        case ARM_tLDMIA: {
            ...
            if (MI->csh->detail) {
                ...
                MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].access =
...
static void printOperand(MCInst *MI, unsigned OpNo, SStream *O) {
    ...
    if (MCOperand_isReg(Op)) {
        ...
        if (MI->csh->detail) {
            if (MI->csh->doing_mem) {
                if (MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].mem.base ==
                    ARM_REG_INVALID)
...
static void printSORegRegOperand(MCInst *MI, unsigned OpNum, SStream *O) {
    ...
    if (MI->csh->detail) {
        ...
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].shift.type =
                (MCOperand_getImm(MO3) & 7) + ARM_SFT_ASR_REG - 1;
        MI->flat_insn->detail->arm.op_count++;
    }
    ...
    if (MI->csh->detail)
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count - 1].shift.value = MCOperand_getReg(
                MO2);
}
...
static void printAM2PreOrOffsetIndexOp(MCInst *MI, unsigned Op, SStream *O) {
    ...
    if (!MCOperand_getReg(MO2)) {
        ...
        if (tmp) { // Don't print +0.
            ...
            if (MI->csh->detail) {
                MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].shift.type = (arm_shifter) getAM2Op(
                        imm3);
                MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].shift.value = tmp;
                MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].subtracted =
...
static void printGPRPairOperand(MCInst *MI, unsigned OpNum, SStream *O) {
    ...
    if (MI->csh->detail) {
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].type = ARM_OP_REG;
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].reg = MCRegisterInfo_getSubReg(MI->MRI,
                                                                                                                Reg,
                                                                                                                ARM_gsub_0);
        MI->flat_insn->detail->arm.op_count++;
    }
...
static void printFPImmOperand(MCInst *MI, unsigned OpNum, SStream *O) {
    ...
    if (MI->csh->detail) {
        ...
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].fp = getFPImmFloat(
                (unsigned int) MCOperand_getImm(MO));
        MI->flat_insn->detail->arm.op_count++;
    }
}
...
void printMVEVectorList(MCInst *MI, unsigned OpNum, SStream *O, unsigned NumRegs) {
    unsigned Reg = MCOperand_getReg(MCInst_getOperand(MI, OpNum));
    for (unsigned i = 0; i < NumRegs; i++) {
        SStream_concat1(O, '{');
        printRegName(MI->csh, O, MCRegisterInfo_getSubReg(MI->MRI, Reg, ARM_qsub_0 + i));
        SStream_concat0(O, ", ");
    }
    SStream_concat1(O, '}');
}
...
static void printVectorListThree(MCInst *MI, unsigned OpNum, SStream *O) {
    ...
    if (MI->csh->detail) {
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].type = ARM_OP_REG;
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].reg =
    ...
    if (MI->csh->detail) {
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].type = ARM_OP_REG;
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].reg =
                MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 2;
#ifndef CAPSTONE_DIET
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].access = access;
#endif
        MI->flat_insn->detail->arm.op_count++;
...
static void printVectorListFour(MCInst *MI, unsigned OpNum, SStream *O) {
    ...
    if (MI->csh->detail) {
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].type = ARM_OP_REG;
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].reg =
                MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 3;
#ifndef CAPSTONE_DIET
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].access = access;
#endif
        MI->flat_insn->detail->arm.op_count++;
...
static void printVectorListThreeSpacedAllLanes(MCInst *MI,
    ...
    if (MI->csh->detail) {
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].type = ARM_OP_REG;
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].reg =
                MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 4;
#ifndef CAPSTONE_DIET
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].access = access;
#endif
        MI->flat_insn->detail->arm.op_count++;
...
static void printVectorListFourSpacedAllLanes(MCInst *MI,
    ...
    if (MI->csh->detail) {
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].type = ARM_OP_REG;
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].reg =
                MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 6;
#ifndef CAPSTONE_DIET
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count].access = access;
#endif
        MI->flat_insn->detail->arm.op_count++;
