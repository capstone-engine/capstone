...
static void printAddrMode3Operand(MCInst *MI, unsigned OpNum, SStream *O,
...
static void printBitfieldInvMaskImmOperand(MCInst *MI, unsigned OpNum,
                                           SStream *O);
...
static void printAdrLabelOperand(MCInst *MI, unsigned OpNum, SStream *O,
                                 unsigned);
...
static void printThumbAddrModeImm5SOperand(MCInst *MI, unsigned OpNum,
                                           SStream *O, unsigned Scale);

static void printThumbAddrModeImm5S1Operand(MCInst *MI, unsigned OpNum,
                                            SStream *O);

static void printThumbAddrModeImm5S2Operand(MCInst *MI, unsigned OpNum,
                                            SStream *O);

static void printThumbAddrModeImm5S4Operand(MCInst *MI, unsigned OpNum,
                                            SStream *O);
...
static void printAddrModeImm12Operand(MCInst *MI, unsigned OpNum, SStream *O,
                                      bool AlwaysPrintImm0);

static void printT2AddrModeImm8Operand(MCInst *MI, unsigned OpNum, SStream *O,
                                       bool);

static void printT2AddrModeImm8s4Operand(MCInst *MI, unsigned OpNum, SStream *O,
                                         bool);

static void printT2AddrModeImm0_1020s4Operand(MCInst *MI, unsigned OpNum,
                                              SStream *O);

static void printT2AddrModeImm8OffsetOperand(MCInst *MI, unsigned OpNum,
                                             SStream *O);

static void printT2AddrModeImm8s4OffsetOperand(MCInst *MI, unsigned OpNum,
                                               SStream *O);
...
static void printMandatoryPredicateOperand(MCInst *MI, unsigned OpNum,
                                           SStream *O);
...
static void printVectorListThreeAllLanes(MCInst *MI, unsigned OpNum,
                                         SStream *O);

static void printVectorListFourAllLanes(MCInst *MI, unsigned OpNum, SStream *O);

static void printVectorListTwoSpacedAllLanes(MCInst *MI, unsigned OpNum,
                                             SStream *O);

static void printVectorListThreeSpacedAllLanes(MCInst *MI, unsigned OpNum,
                                               SStream *O);

static void printVectorListFourSpacedAllLanes(MCInst *MI, unsigned OpNum,
                                              SStream *O);
...
static void printVPTPredicateOperand(MCInst *MI, unsigned OpNum, SStream *O);

static void printVPTMask(MCInst *MI, unsigned OpNum, SStream *O);

static void printMVEVectorList(MCInst *MI, unsigned NumRegs, SStream *O,
                               unsigned OpNum);

static void printVMOVModImmOperand(MCInst *MI, unsigned OpNum, SStream *O);

static void printMveAddrModeRQOperand(MCInst *MI, unsigned OpNum, SStream *O,
                                      int shift);

static void printMandatoryRestrictedPredicateOperand(MCInst *MI, unsigned OpNum,
                                                     SStream *O);

static void printMandatoryInvertedPredicateOperand(MCInst *MI, unsigned OpNum,
                                                   SStream *O);

static void printMveSaturateOp(MCInst *MI, unsigned OpNum, SStream *O);

// -- mark the line of inserted print functions
...
static uint8_t get_op_access(cs_struct *h, unsigned int id,
                             unsigned int index) {
...
static void set_mem_access(MCInst *MI, bool status) {
  ...
  if (status) {
    ...
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_MEM;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
...
const char *getRegisterNameRaw(unsigned RegNo) {
  return getRegisterName(RegNo, ARM_RegNamesRaw);
}
...
void ARM_getRegName(cs_struct *handle, int value) {
  if (value == CS_OPT_SYNTAX_NOREGNAME) {
    ...
    handle->get_regname = getRegisterNameNoRaw;
    handle->reg_name = ARM_reg_name;
    debugln("using non raw");
  }
}
...
static void printRegImmShift(MCInst *MI, SStream *O, ARM_AM_ShiftOpc ShOpc,
  ...
  if (MI->csh->detail) {
    if (MI->csh->doing_mem)
      MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
          .shift.type = (arm_shifter)ShOpc;
    else
      MI->flat_insn->detail->arm
          .operands[MI->flat_insn->detail->arm.op_count - 1]
          .shift.type = (arm_shifter)ShOpc;
  }
  ...
  if (ShOpc != ARM_AM_rrx) {
    ...
    if (MI->csh->detail) {
      if (MI->csh->doing_mem)
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
            .shift.value = translateShiftImm(ShImm);
      else
        MI->flat_insn->detail->arm
            .operands[MI->flat_insn->detail->arm.op_count - 1]
            .shift.value = translateShiftImm(ShImm);
...
static const name_map insn_update_flgs[] = {
    {ARM_INS_CMN, "cmn"},      {ARM_INS_CMP, "cmp"},
    {ARM_INS_TEQ, "teq"},      {ARM_INS_TST, "tst"},

    {ARM_INS_ADC, "adcs"},     {ARM_INS_ADD, "adds"},
    {ARM_INS_AND, "ands"},     {ARM_INS_ASR, "asrs"},
    {ARM_INS_BIC, "bics"},     {ARM_INS_EOR, "eors"},
    {ARM_INS_LSL, "lsls"},     {ARM_INS_LSR, "lsrs"},
    {ARM_INS_MLA, "mlas"},     {ARM_INS_MOV, "movs"},
    {ARM_INS_MUL, "muls"},     {ARM_INS_MVN, "mvns"},
    {ARM_INS_ORN, "orns"},     {ARM_INS_ORR, "orrs"},
    {ARM_INS_ROR, "rors"},     {ARM_INS_RRX, "rrxs"},
    {ARM_INS_RSB, "rsbs"},     {ARM_INS_RSC, "rscs"},
    {ARM_INS_SBC, "sbcs"},     {ARM_INS_SMLAL, "smlals"},
    {ARM_INS_SMULL, "smulls"}, {ARM_INS_SUB, "subs"},
    {ARM_INS_UMLAL, "umlals"}, {ARM_INS_UMULL, "umulls"},
...
void ARM_printInst(MCInst *MI, SStream *O, void *Info) {
  ...
  case ARM_MOVsi: {
    ...
    SStream_concat0(O, ", ");
    tmp =
        translateShiftImm(getSORegOffset((unsigned int)MCOperand_getImm(MO2)));
    printUInt32Bang(O, tmp);
    if (MI->csh->detail) {
      MI->flat_insn->detail->arm
          .operands[MI->flat_insn->detail->arm.op_count - 1]
          .shift.type = (arm_shifter)opc;
      MI->flat_insn->detail->arm
          .operands[MI->flat_insn->detail->arm.op_count - 1]
          .shift.value = tmp;
    }
  ...
  case ARM_t2STMDB_UPD:
    if (MCOperand_getReg(MCInst_getOperand(MI, 0)) == ARM_SP &&
      ...
      if (MI->csh->detail) {
        MI->flat_insn->detail
            ->regs_read[MI->flat_insn->detail->regs_read_count] = ARM_REG_SP;
        MI->flat_insn->detail->regs_read_count++;
        MI->flat_insn->detail
            ->regs_write[MI->flat_insn->detail->regs_write_count] = ARM_REG_SP;
  ...
  case ARM_LDR_POST_IMM:
    if (MCOperand_getReg(MCInst_getOperand(MI, 2)) == ARM_SP) {
      ...
      if (getAM2Offset((unsigned int)MCOperand_getImm(MO2)) == 4) {
        ...
        if (MI->csh->detail) {
          MI->flat_insn->detail->arm
              .operands[MI->flat_insn->detail->arm.op_count]
              .type = ARM_OP_REG;
          MI->flat_insn->detail->arm
              .operands[MI->flat_insn->detail->arm.op_count]
  ...
  case ARM_STLEXD: {
    ...
    if (MCRegisterClass_contains(MRC, Reg)) {
      ...
      MCOperand_CreateReg0(
          &NewMI, MCRegisterInfo_getMatchingSuperReg(
                      MRI, Reg, ARM_gsub_0,
...
static void printSORegRegOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MI->csh->detail) {
    ...
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .shift.type = (MCOperand_getImm(MO3) & 7) + ARM_SFT_ASR_REG - 1;
    MI->flat_insn->detail->arm.op_count++;
  }
  ...
  if (MI->csh->detail)
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count - 1]
        .shift.value = MCOperand_getReg(MO2);
}
...
static void printSORegImmOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  // Print the shift opc.
  printRegImmShift(MI, O,
                   ARM_AM_getSORegShOp((unsigned int)MCOperand_getImm(MO2)),
...
static void printAM2PreOrOffsetIndexOp(MCInst *MI, unsigned Op, SStream *O) {
  ...
  if (!MCOperand_getReg(MO2)) {
    ...
    if (tmp) { // Don't print +0.
      ...
      if (MI->csh->detail) {
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
            .shift.type = (arm_shifter)getAM2Op(imm3);
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
            .shift.value = tmp;
        MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
            .subtracted = subtracted == ARM_AM_sub;
      }
    }
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .mem.index = MCOperand_getReg(MO2);
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .subtracted = subtracted == ARM_AM_sub;
  }
...
static void printAddrMode2Operand(MCInst *MI, unsigned Op, SStream *O) {
  ...
  //  assert(IdxMode != ARMII::IndexModePost && "Should be pre or offset index
  //  op");
  //#endif
...
static void printAddrMode2OffsetOperand(MCInst *MI, unsigned OpNum,
                                        SStream *O) {
  MCOperand *MO1 = MCInst_getOperand(MI, OpNum);
  MCOperand *MO2 = MCInst_getOperand(MI, OpNum + 1);
...
static void printAddrMode3OffsetOperand(MCInst *MI, unsigned OpNum,
                                        SStream *O) {
  MCOperand *MO1 = MCInst_getOperand(MI, OpNum);
  MCOperand *MO2 = MCInst_getOperand(MI, OpNum + 1);
...
static void printAddrMode5Operand(MCInst *MI, unsigned OpNum, SStream *O,
  ...
  if (AlwaysPrintImm0 || ImmOffs || Op == ARM_AM_sub) {
    if (ImmOffs * 4 > HEX_THRESHOLD)
      SStream_concat(O, ", #%s0x%x", ARM_AM_getAddrOpcStr(Op), ImmOffs * 4);
    else
      SStream_concat(O, ", #%s%u", ARM_AM_getAddrOpcStr(Op), ImmOffs * 4);
...
static void printAddrMode6OffsetOperand(MCInst *MI, unsigned OpNum,
                                        SStream *O) {
  MCOperand *MO = MCInst_getOperand(MI, OpNum);
...
static void printShiftImmOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  unsigned ShiftOp =
      (unsigned int)MCOperand_getImm(MCInst_getOperand(MI, OpNum));
...
// FIXME: push {r1, r2, r3, ...} can exceed the number of operands in MCInst
// struct
static void printRegisterList(MCInst *MI, unsigned OpNum, SStream *O) {
  unsigned i, e;
#ifndef CAPSTONE_DIET
...
static void printGPRPairOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_REG;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .reg = MCRegisterInfo_getSubReg(MI->MRI, Reg, ARM_gsub_0);
    MI->flat_insn->detail->arm.op_count++;
  }
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_REG;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .reg = MCRegisterInfo_getSubReg(MI->MRI, Reg, ARM_gsub_1);
    MI->flat_insn->detail->arm.op_count++;
  }
}
...
static void printThumbS4ImmOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  unsigned tmp =
      (unsigned int)MCOperand_getImm(MCInst_getOperand(MI, OpNum)) * 4;
...
static void printThumbITMask(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  unsigned Firstcond =
      (unsigned int)MCOperand_getImm(MCInst_getOperand(MI, OpNum - 1));
...
static void printThumbAddrModeImm5S1Operand(MCInst *MI, unsigned Op,
                                            SStream *O) {
  printThumbAddrModeImm5SOperand(MI, Op, O, 1);
}

static void printThumbAddrModeImm5S2Operand(MCInst *MI, unsigned Op,
                                            SStream *O) {
  printThumbAddrModeImm5SOperand(MI, Op, O, 2);
}

static void printThumbAddrModeImm5S4Operand(MCInst *MI, unsigned Op,
                                            SStream *O) {
...
static void printT2AddrModeSoRegOperand(MCInst *MI, unsigned OpNum,
                                        SStream *O) {
  MCOperand *MO1 = MCInst_getOperand(MI, OpNum);
  MCOperand *MO2 = MCInst_getOperand(MI, OpNum + 1);
  MCOperand *MO3 = MCInst_getOperand(MI, OpNum + 2);
  ...
  // assert(MCOperand_getReg(MO2.getReg() && "Invalid so_reg load / store
  // address!");
  SStream_concat0(O, ", ");
  printRegName(MI->csh, O, MCOperand_getReg(MO2));
...
static void printFPImmOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_FP;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .fp = getFPImmFloat((unsigned int)MCOperand_getImm(MO));
    MI->flat_insn->detail->arm.op_count++;
  }
}
...
static void printVPTPredicateOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  VPTCodes CCCode = MCOperand_getImm(MCInst_getOperand(MI, OpNum));

  if (CCCode != ARMVCC_None)
    SStream_concat0(O, ARMVPTPredToString(CCCode));
}
...
void printMVEVectorList(MCInst *MI, unsigned OpNum, SStream *O,
                        unsigned NumRegs) {
  unsigned Reg = MCOperand_getReg(MCInst_getOperand(MI, OpNum));
  for (unsigned i = 0; i < NumRegs; i++) {
    SStream_concat1(O, '{');
    printRegName(MI->csh, O,
...
static void printMveAddrModeRQOperand(MCInst *MI, unsigned OpNum, SStream *O,
                                      int shift) {
  unsigned MO1 = MCOperand_getReg(MCInst_getOperand(MI, OpNum));
  unsigned MO2 = MCOperand_getReg(MCInst_getOperand(MI, OpNum + 1));

  SStream_concat0(O, "[");
  printRegName(MI->csh, O, MO1);
  SStream_concat0(O, ", ");
  printRegName(MI->csh, O, MO2);
...
static void printMandatoryRestrictedPredicateOperand(MCInst *MI, unsigned OpNum,
                                                     SStream *O) {
  if (MCOperand_getImm(MCInst_getOperand(MI, OpNum)) == ARMCC_HS) {
...
static void printMandatoryInvertedPredicateOperand(MCInst *MI, unsigned OpNum,
                                                   SStream *O) {
  ARMCC_CondCodes CC = MCOperand_getImm(MCInst_getOperand(MI, OpNum));
  SStream_concat0(O, ARMCC_ARMCondCodeToString(ARMCC_getOppositeCondition(CC)));
}

static void printMveSaturateOp(MCInst *MI, unsigned OpNum, SStream *O) {
  uint32_t Val = MCOperand_getImm(MCInst_getOperand(MI, OpNum));
  SStream_concat0(O, "#");
  SStream_concat0(O, Val == 1 ? "48" : "64");
}
...
static void printVectorListThree(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_REG;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .reg = MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 1;
#ifndef CAPSTONE_DIET
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .access = access;
#endif
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_REG;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .reg = MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 2;
#ifndef CAPSTONE_DIET
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .access = access;
#endif
...
static void printVectorListFour(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_REG;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .reg = MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 3;
#ifndef CAPSTONE_DIET
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .access = access;
#endif
...
static void printVectorListFourAllLanes(MCInst *MI, unsigned OpNum,
                                        SStream *O) {
#ifndef CAPSTONE_DIET
  uint8_t access;
...
static void printVectorListThreeSpacedAllLanes(MCInst *MI, unsigned OpNum,
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_REG;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .reg = MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 4;
#ifndef CAPSTONE_DIET
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .access = access;
#endif
...
static void printVectorListFourSpacedAllLanes(MCInst *MI, unsigned OpNum,
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .type = ARM_OP_REG;
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .reg = MCOperand_getReg(MCInst_getOperand(MI, OpNum)) + 6;
#ifndef CAPSTONE_DIET
    MI->flat_insn->detail->arm.operands[MI->flat_insn->detail->arm.op_count]
        .access = access;
#endif
