...
typedef enum VPTCodes { ARMVCC_None = 0, ARMVCC_Then, ARMVCC_Else } VPTCodes;

inline static const char *ARMVPTPredToString(VPTCodes CC) {
  switch (CC) {
  case ARMVCC_None:
    return "none";
  case ARMVCC_Then:
...
typedef enum ARMCC_CondCodes { // Meaning (integer)          Meaning
                               // (floating-point)
...
static inline const MClassSysReg *
