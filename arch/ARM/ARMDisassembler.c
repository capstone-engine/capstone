...
#define DECLARE_DecodeT2Imm7(shift)                                            \
	static DecodeStatus CONCAT(DecodeT2Imm7, shift)(                           \
...
#define DECLARE_DecodeT2AddrModeImm7(shift, WriteBack)                         \
	...
	DECLARE_DecodeT2AddrModeImm7(0, 0) DECLARE_DecodeT2AddrModeImm7(1, 0)
		DECLARE_DecodeT2AddrModeImm7(2, 0) DECLARE_DecodeT2AddrModeImm7(0, 1)
...
#define DECLARE_DecodeMVE_MEM_2_pre(shift)                                     \
	static DecodeStatus CONCAT(DecodeMVE_MEM_2_pre, shift)(                    \
		MCInst * Inst, unsigned Val, uint64_t Address, const void *Decoder);
		DECLARE_DecodeMVE_MEM_2_pre(0) DECLARE_DecodeMVE_MEM_2_pre(1)
...
#define DECLARE_DecodeMVE_MEM_3_pre(shift)                                     \
	static DecodeStatus CONCAT(DecodeMVE_MEM_3_pre, shift)(                    \
		MCInst * Inst, unsigned Val, uint64_t Address, const void *Decoder);
				DECLARE_DecodeMVE_MEM_3_pre(2) DECLARE_DecodeMVE_MEM_3_pre(3)

#define DECLARE_DecodePowerTwoOperand(MinLog, MaxLog)                          \
	static DecodeStatus CONCAT(DecodePowerTwoOperand, CONCAT(MinLog, MaxLog))( \
		MCInst * Inst, unsigned Val, uint64_t Address, const void *Decoder);
				...
				DECLARE_DecodeMVEVCMP(true, DecodeRestrictedUPredicateOperand)
					DECLARE_DecodeMVEVCMP(true,
						...
						DECLARE_DecodeMVEVCMP(
							false, DecodeRestrictedFPPredicateOperand)
							DECLARE_DecodeMVEVCMP(
								true, DecodeRestrictedFPPredicateOperand)
							...
							void *Info)
{
	...
	if (MI->csh->mode & CS_MODE_THUMB)
		Result =
			getThumbInstruction(ud, Bytes, BytesLen, MI, Size, Address, Info);
	else
		Result =
			getARMInstruction(ud, Bytes, BytesLen, MI, Size, Address, Info);
	...
	for (i = 0; i < NumOps; ++i) {
		...
		if (MCOperandInfo_isOptionalDef(&OpInfo[i]) &&
			...
			if (i > 0 && MCOperandInfo_isPredicate(&OpInfo[i - 1]))
				continue;
			MCInst_insert0(
				MI, i, MCOperand_CreateReg1(MI, (InITBlock ? 0 : ARM_CPSR)));
			return;
		}
	}
	...
	if (isVectorPredicable(MCInst_getOpcode(MI))) {
		...
		if (OpInfo[VCCPos].OperandType == ARM_OP_VPRED_R) {
			int TiedOp = MCOperandInfo_getOperandConstraint(
				&ARMInsts[MCInst_getOpcode(MI)], VCCPos + 3, MCOI_TIED_TO);
			assert(TiedOp >= 0 &&
...
void UpdateThumbVFPPredicate(DecodeStatus S, MCInst *MI)
{
	...
	for (unsigned i = 0; i < NumOps; ++i) {
		if (MCOperandInfo_isPredicate(&OpInfo[i])) {
			if (CC != ARMCC_AL &&
				!MCInst_isPredicable(&ARMInsts[MCInst_getOpcode(MI)]))
				Check(&S, MCDisassembler_SoftFail);
								 ...
								 void *Info)
{
	...
	if (Result != MCDisassembler_Fail) {
		*Size = 4;
		...
		if (isVPTOpcode(MCInst_getOpcode(MI))) {
			unsigned Mask = MCOperand_getImm(MCInst_getOperand(MI, (0)));
	...
	if (fieldFromInstruction_4(Insn32, 28, 4) == 0xE) {
		Result =
			decodeInstruction_4(DecoderTableNEONDup32, MI, Insn32, Address);
		if (Result != MCDisassembler_Fail) {
			*Size = 4;
			Check(&Result, AddThumbPredicate(MI));
			return Result;
	...
	if (fieldFromInstruction_4(Insn32, 24, 4) == 0xF) {
		...
		NEONDataInsn |=
			(NEONDataInsn & 0x10000000) >> 4; // Move bit 28 to bit 24
		...
		Result =
			decodeInstruction_4(DecoderTablev8NEON32, MI, NEONv8Insn, Address);
		if (Result != MCDisassembler_Fail) {
			*Size = 4;
			return Result;
		}
											...
											const void *Decoder)
{
	...
	if ((RegNo == 13 && !ARM_getFeatureBits(Inst->csh->mode, ARM_HasV8Ops)) ||
		RegNo == 15)
		S = MCDisassembler_SoftFail;
										 ...
										 uint64_t Address, const void *Decoder)
{
	...
	for (unsigned i = 0; i < 16; ++i) {
		if (Val & (1 << i)) {
			if (CLRM) {
				if (!Check(&S, DecodeCLRMGPRRegisterClass(Inst, i, Address,
														  Decoder))) {
					return MCDisassembler_Fail;
				}
			} else {
				...
				if (NeedDisjointWriteback &&
					WritebackReg ==
						MCOperand_getReg(&(Inst->Operands[Inst->size - 1])))
					Check(&S, MCDisassembler_SoftFail);
			}
		}
	}
											...
											const void *Decoder)
{
	...
	case ARM_STC2L_POST:
		if (coproc == 0xA || coproc == 0xB ||
			(ARM_getFeatureBits(Inst->csh->mode, ARM_HasV8_1MMainlineOps) &&
											   ...
											   const void *Decoder)
{
	...
	if (U)
		MCOperand_CreateImm0(Inst, (ARM_AM_getAM5FP16Opc(ARM_AM_add, imm)));
	else
		MCOperand_CreateImm0(Inst, (ARM_AM_getAM5FP16Opc(ARM_AM_sub, imm)));
											   ...
											   const void *Decoder)
{
	...
	if (!tryAddingSymbolicOperand(Address,
								  Address + SignExtend32((imm), 26) + 8, true,
								  4, Inst, Decoder))
		MCOperand_CreateImm0(Inst, (SignExtend32((imm), 26)));
										 ...
										 uint64_t Address, const void *Decoder)
{
	...
	case ARM_VLD2b8wb_register:
		if (!Check(&S,
				   DecodeDPairSpacedRegisterClass(Inst, Rd, Address, Decoder)))
			return MCDisassembler_Fail;
		break;
	default:
...
{
	if (!tryAddingSymbolicOperand(Address,
								  Address + SignExtend32((Val << 1), 12) + 4,
								  true, 2, Inst, Decoder))
...
{
	if (!tryAddingSymbolicOperand(Address,
								  Address + SignExtend32((Val), 21) + 4, true,
								  4, Inst, Decoder))
...
#define DEFINE_DecodeTAddrModeImm7(shift)                                      \
	...
	{                                                                          \
		...
		if (!Check(&S, DecodetGPRRegisterClass(Inst, Rn, Address, Decoder)))   \
			return MCDisassembler_Fail;                                        \
		if (!Check(&S,                                                         \
				   CONCAT(DecodeT2Imm7, shift)(Inst, imm, Address, Decoder)))  \
			return MCDisassembler_Fail;                                        \
                                                                               \
		return S;                                                              \
	}
...
#define DEFINE_DecodeT2AddrModeImm7(shift, WriteBack)                          \
	...
	{                                                                          \
		...
		if (WriteBack) {                                                       \
			if (!Check(&S,                                                     \
					   DecoderGPRRegisterClass(Inst, Rn, Address, Decoder)))   \
				return MCDisassembler_Fail;                                    \
	...
	DEFINE_DecodeT2AddrModeImm7(0, 0) DEFINE_DecodeT2AddrModeImm7(1, 0)
		DEFINE_DecodeT2AddrModeImm7(2, 0) DEFINE_DecodeT2AddrModeImm7(0, 1)
			DEFINE_DecodeT2AddrModeImm7(1, 1) DEFINE_DecodeT2AddrModeImm7(2, 1)
								  ...
								  const void *Decoder)
{
	...
	if (ARM_getFeatureBits(Inst->csh->mode, ARM_FeatureMClass)) {
		...
		case 0xa7: // pac_key_u_3_ns
			if (!(ARM_getFeatureBits(Inst->csh->mode, ARM_FeaturePACBTI)))
				return MCDisassembler_Fail;
			break;
		default:
								 ...
								 const void *Decoder)
{
	...
	if (!Check(&S,
		...
		return MCDisassembler_Fail;
	if (!Check(&S,
			   DecodeDPRRegisterClass(Inst, Rd + 3 * inc, Address, Decoder)))
		return MCDisassembler_Fail;
													   ...
													   const void *Decoder)
{
	...
	typedef DecodeStatus (*DecoderFunction)(
		MCInst * Inst, unsigned RegNo, uint64_t Address, const void *Decoder);

	DecoderFunction DestRegDecoder =
		q ? DecodeQPRRegisterClass : DecodeDPRRegisterClass;
								 ...
								 const void *Decoder)
{
	...
	case ARM_t2LE:
		if (!Check(&S, CONCAT(DecodeBFLabelOperand,
							  CONCAT(false, CONCAT(true, CONCAT(true, 11))))(
	...
	case ARM_MVE_WLSTP_64:
		...
		if (!Check(&S, DecoderGPRRegisterClass(
			...
			!Check(&S, CONCAT(DecodeBFLabelOperand,
							  CONCAT(false, CONCAT(false, CONCAT(true, 11))))(
						   Inst, Imm, Address, Decoder)))
			return MCDisassembler_Fail;
		break;
	case ARM_t2DLS:
								  ...
								  const void *Decoder)
{
	...
	if (MCInst_getOpcode(Inst) == ARM_VSCCLRMD) {
		...
		if (!Check(&S,
				   DecodeDPRRegListOperand(Inst, reglist, Address, Decoder))) {
			return MCDisassembler_Fail;
		}
	} else {
		...
		if (!Check(&S,
				   DecodeSPRRegListOperand(Inst, reglist, Address, Decoder))) {
			return MCDisassembler_Fail;
		}
	}
	MCOperand_CreateReg0(Inst, (ARM_VPR));
...
#define DEFINE_DecodeVSTRVLDR_SYSREG(Writeback)                                \
	...
	{                                                                          \
		...
		case ARM_VLDR_FPSCR_NZCVQC_post:                                       \
                                                                               \
			if (!ARM_getFeatureBits(Inst->csh->mode, ARM_HasMVEIntegerOps) &&  \
				!ARM_getFeatureBits(Inst->csh->mode, ARM_FeatureVFP2))         \
				return MCDisassembler_Fail;                                    \
		}                                                                      \
                                                                               \
		DecodeStatus S = MCDisassembler_Success;                               \
		unsigned Sysreg = FixedRegForVSTRVLDR_SYSREG(MCInst_getOpcode(Inst));  \
		if (Sysreg)                                                            \
		...
		unsigned addr = fieldFromInstruction_4(Val, 0, 7) |                    \
						(fieldFromInstruction_4(Val, 23, 1) << 7) | (Rn << 8); \
                                                                               \
		if (Writeback) {                                                       \
			if (!Check(&S, DecodeGPRnopcRegisterClass(Inst, Rn, Address,       \
													  Decoder)))               \
				return MCDisassembler_Fail;                                    \
		}                                                                      \
...
#define DEFINE_DecodeMVE_MEM_1_pre(shift)                                      \
	...
	{                                                                          \
		return DecodeMVE_MEM_pre(                                              \
			Inst, Val, Address, Decoder, fieldFromInstruction_4(Val, 16, 3),   \
			DecodetGPRRegisterClass, CONCAT(DecodeTAddrModeImm7, shift));      \
	}
...
#define DEFINE_DecodeMVE_MEM_2_pre(shift)                                      \
	...
	{                                                                          \
		return DecodeMVE_MEM_pre(                                              \
			Inst, Val, Address, Decoder, fieldFromInstruction_4(Val, 16, 4),   \
			DecoderGPRRegisterClass,                                           \
			CONCAT(DecodeT2AddrModeImm7, CONCAT(shift, 1)));                   \
	}
	DEFINE_DecodeMVE_MEM_2_pre(0) DEFINE_DecodeMVE_MEM_2_pre(1)
...
#define DEFINE_DecodeMVE_MEM_3_pre(shift)                                      \
	...
	{                                                                          \
		return DecodeMVE_MEM_pre(                                              \
			Inst, Val, Address, Decoder, fieldFromInstruction_4(Val, 17, 3),   \
			DecodeMQPRRegisterClass, CONCAT(DecodeMveAddrModeQ, shift));       \
	}
...
#define DEFINE_DecodeMVEVCMP(scalar, predicate_decoder)                        \
	...
	{                                                                          \
		...
		if (scalar) {                                                          \
			...
			unsigned Rm = fieldFromInstruction_4(Insn, 0, 4);                  \
			if (!Check(&S, DecodeGPRwithZRRegisterClass(Inst, Rm, Address,     \
														Decoder)))             \
				return MCDisassembler_Fail;                                    \
		} else {                                                               \
			fc = fieldFromInstruction_4(Insn, 12, 1) << 2 |                    \
			...
			unsigned Qm = fieldFromInstruction_4(Insn, 5, 1) << 4 |            \
						  fieldFromInstruction_4(Insn, 1, 3);                  \
			if (!Check(&S,                                                     \
					   DecodeMQPRRegisterClass(Inst, Qm, Address, Decoder)))   \
				return MCDisassembler_Fail;                                    \
		}                                                                      \
                                                                               \
										...
										uint64_t Address, const void *Decoder)
{
	...
	if (TypeT3) {
		...
		if (!Check(&DS, DecodeT2SOImm(Inst, Imm12, Address, Decoder))) // imm12
			return MCDisassembler_Fail;
		if (!Check(&DS,
				   DecodeCCOutOperand(Inst, S, Address, Decoder))) // cc_out
			return MCDisassembler_Fail;
	}
