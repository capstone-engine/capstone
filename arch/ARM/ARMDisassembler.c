...
void ARM_init(MCRegisterInfo *MRI) {
  ...
  MCRegisterInfo_InitMCRegisterInfo(
      MRI, ARMRegDesc, ARR_SIZE(ARMRegDesc), 0, 0, ARMMCRegisterClasses,
      ARR_SIZE(ARMMCRegisterClasses), 0, 0, ARMRegDiffLists, 0,
      ARMSubRegIdxLists, ARR_SIZE(ARMSubRegIdxLists), 0);
}
...
static DecodeStatus _ARM_getInstruction(cs_struct *ud, MCInst *MI,
  ...
  *Size = 0;
  ...
  MCInst_clear(MI);
  result = decodeInstruction_4(DecoderTableNEONData32, MI, insn, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 4;
  ...
  MCInst_clear(MI);
  result = decodeInstruction_4(DecoderTableNEONDup32, MI, insn, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 4;
    // Add a fake predicate operand, because we share these instruction
  ...
  MCInst_clear(MI);
  result = decodeInstruction_4(DecoderTablev8Crypto32, MI, insn, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 4;
    return result;
...
static DecodeStatus AddThumbPredicate(cs_struct *ud, MCInst *MI) {
  ...
  case ARM_t2TBH:
    ...
    if (ITStatus_instrInITBlock(&(ud->ITBlock)) &&
        !ITStatus_instrLastInITBlock(&(ud->ITBlock)))
      S = MCDisassembler_SoftFail;
    break;
  default:
    break;
...
static DecodeStatus _Thumb_getInstruction(cs_struct *ud, MCInst *MI,
                                          const uint8_t *code, size_t code_len,
                                          uint16_t *Size, uint64_t Address) {
  uint16_t insn16;
  DecodeStatus result;
  ...
  debugln("table 16");
  result = decodeInstruction_2(DecoderTableThumb16, MI, insn16, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 2;
  ...
  debugln("table s16");
  MCInst_clear(MI);
  result = decodeInstruction_2(DecoderTableThumbSBit16, MI, insn16, Address, 0,
                               ud->mode);
  if (result) {
    *Size = 2;
  ...
  MCInst_clear(MI);
  debugln("table 216");
  result = decodeInstruction_2(DecoderTableThumb216, MI, insn16, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 2;
    ...
    if (MCInst_getOpcode(MI) == ARM_t2IT &&
        ITStatus_instrInITBlock(&(ud->ITBlock)))
      return MCDisassembler_SoftFail;
  ...
  if (MODE_IS_BIG_ENDIAN(ud->mode))
    insn32 = (code[3] << 0) | (code[2] << 8) | (code[1] << 16) |
             ((uint32_t)code[0] << 24);
  else
    insn32 = (code[3] << 8) | (code[2] << 0) | ((uint32_t)code[1] << 24) |
             (code[0] << 16);

  MCInst_clear(MI);
  debugln("table 32");
  result = decodeInstruction_4(DecoderTableThumb32, MI, insn32, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 4;
    InITBlock = ITStatus_instrInITBlock(&(ud->ITBlock));
  ...
  MCInst_clear(MI);
  debugln("table 232");
  result = decodeInstruction_4(DecoderTableThumb232, MI, insn32, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 4;
    Check(&result, AddThumbPredicate(ud, MI));
  ...
  if (fieldFromInstruction_4(insn32, 28, 4) == 0xE) {
    MCInst_clear(MI);
    result = decodeInstruction_4(DecoderTableVFP32, MI, insn32, Address, 0,
                                 ud->mode);
    if (result != MCDisassembler_Fail) {
      *Size = 4;
  ...
  MCInst_clear(MI);
  debugln("table vfpv");
  result = decodeInstruction_4(DecoderTableVFPV832, MI, insn32, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 4;
    return result;
  ...
  if (fieldFromInstruction_4(insn32, 28, 4) == 0xE) {
    MCInst_clear(MI);
    result = decodeInstruction_4(DecoderTableNEONDup32, MI, insn32, Address, 0,
                                 ud->mode);
    if (result != MCDisassembler_Fail) {
      *Size = 4;
      Check(&result, AddThumbPredicate(ud, MI));
  ...
  if (fieldFromInstruction_4(insn32, 24, 8) == 0xF9) {
    ...
    result = decodeInstruction_4(DecoderTableNEONLoadStore32, MI, NEONLdStInsn,
                                 Address, 0, ud->mode);
    if (result != MCDisassembler_Fail) {
      *Size = 4;
      Check(&result, AddThumbPredicate(ud, MI));
  ...
  if (fieldFromInstruction_4(insn32, 24, 4) == 0xF) {
    ...
    result = decodeInstruction_4(DecoderTableNEONData32, MI, NEONDataInsn,
                                 Address, 0, ud->mode);
    if (result != MCDisassembler_Fail) {
      *Size = 4;
      Check(&result, AddThumbPredicate(ud, MI));
  ...
  result = decodeInstruction_4(DecoderTablev8NEON32, MI, NEONv8Insn, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 4;
    return result;
  ...
  debugln("table cde32");
  MCInst_clear(MI);
  result = decodeInstruction_4(DecoderTableThumb2CDE32, MI, insn32, Address, 0,
                               ud->mode);
  if (result != MCDisassembler_Fail) {
    *Size = 4;
    Check(&result, AddThumbPredicate(ud, MI));
...
bool Thumb_getInstruction(csh ud, const uint8_t *code, size_t code_len,
                          MCInst *instr, uint16_t *size, uint64_t address,
                          void *info) {
  DecodeStatus status = _Thumb_getInstruction((cs_struct *)ud, instr, code,
                                              code_len, size, address);
...
bool ARM_getInstruction(csh ud, const uint8_t *code, size_t code_len,
                        MCInst *instr, uint16_t *size, uint64_t address,
                        void *info) {
  DecodeStatus status = _ARM_getInstruction((cs_struct *)ud, instr, code,
                                            code_len, size, address);
