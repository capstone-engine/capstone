...
static void printMemExtendImpl(MCInst *MI, bool SignExtend, bool DoShift,
                               unsigned Width, char SrcRegKind, SStream *O);

static void printMemExtend(MCInst *MI, unsigned OpNum, SStream *O,
...
static void printUImm12Offset(MCInst *MI, unsigned OpNum, SStream *O,
                              unsigned Scale);
...
static void printGPRSeqPairsClassOperand(MCInst *MI, unsigned OpNum, SStream *O,
                                         unsigned int size);

static void printVectorList(MCInst *MI, unsigned OpNum, SStream *O,
                            char *LayoutSuffix, MCRegisterInfo *MRI,
                            arm64_vas vas);

static void printTypedVectorList(MCInst *MI, unsigned OpNum, SStream *O,
                                 unsigned NumLanes, char LaneKind);
...
static void printComplexRotationOp(MCInst *MI, unsigned OpNum, SStream *O,
                                   int64_t Angle, int64_t Remainder);
...
static void printExactFPImm(MCInst *MI, unsigned OpNum, SStream *O,
                            unsigned ImmIs0, unsigned ImmIs1);

static void printGPR64as32(MCInst *MI, unsigned OpNum, SStream *O);

// Starting point of new print functions

static void printBarriernXSOption(MCInst *MI, unsigned OpNum, SStream *O);

static void printGPR64x8(MCInst *MI, unsigned OpNum, SStream *O);

static void printSImm(MCInst *MI, unsigned OpNum, SStream *O, int Size);

static void printUnsignedImm(MCInst *MI, unsigned OpNum, SStream *O);
...
static void printMatrixTileVector(MCInst *MI, unsigned OpNum, SStream *O,
                                  bool IsVertical);
...
static void printSVCROp(const MCInst *MI, unsigned OpNum, SStream *O);

#define GET_INSTRINFO_ENUM
#define GET_REGINFO_ENUM
#define GET_ASM_WRITER
#define GET_REGINFO_EXTRA
...
static cs_ac_type get_op_access(cs_struct *h, unsigned int id,
                                unsigned int index) {
...
static void op_addImm(MCInst *MI, int v) {
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count]
        .type = ARM64_OP_IMM;
    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count]
        .imm = v;
...
void AArch64_printInst(MCInst *MI, SStream *O, void *Info) {
  ...
  if (Opcode == AArch64_SBFMXri || Opcode == AArch64_SBFMWri ||
    ...
    if (MCOperand_isImm(Op2) && MCOperand_getImm(Op2) == 0 &&
      ...
      if (AsmMnemonic) {
        SStream_concat(
            ...
            getRegisterName(getWRegFromXReg(MCOperand_getReg(Op1)),
        ...
        if (MI->csh->detail) {
          ...
          uint8_t access;
          access = get_op_access(MI->csh, MCInst_getOpcode(MI), MI->ac_idx);
          MI->flat_insn->detail->arm64
              .operands[MI->flat_insn->detail->arm64.op_count]
              .access = access;
          MI->ac_idx++;
#endif
          MI->flat_insn->detail->arm64
          ...
          MI->flat_insn->detail->arm64
              .operands[MI->flat_insn->detail->arm64.op_count]
              .reg = getWRegFromXReg(MCOperand_getReg(Op1));
          MI->flat_insn->detail->arm64.op_count++;
        }
    ...
    if (MCOperand_getImm(Op2) > MCOperand_getImm(Op3)) {
      ...
      if (MI->csh->detail) {
        ...
        MI->flat_insn->detail->arm64
            .operands[MI->flat_insn->detail->arm64.op_count]
            .imm = (Is64Bit ? 64 : 32) - (int)MCOperand_getImm(Op2);
        MI->flat_insn->detail->arm64.op_count++;
#ifndef CAPSTONE_DIET
        access = get_op_access(MI->csh, MCInst_getOpcode(MI), MI->ac_idx);
        MI->flat_insn->detail->arm64
    ...
    SStream_concat0(O, ", ");
    printInt32Bang(O,
                   (int)MCOperand_getImm(Op3) - (int)MCOperand_getImm(Op2) + 1);
    ...
    if (MI->csh->detail) {
      ...
      MI->flat_insn->detail->arm64
          .operands[MI->flat_insn->detail->arm64.op_count]
          .imm = MCOperand_getImm(Op3) - MCOperand_getImm(Op2) + 1;
      MI->flat_insn->detail->arm64.op_count++;
    }
  ...
  if ((Opcode == AArch64_MOVZXi || Opcode == AArch64_MOVZWi) &&
    ...
    if (isMOVZMovAlias(Value, Shift, Opcode == AArch64_MOVZXi ? 64 : 32)) {
      SStream_concat(O, "mov\t%s, ",
                     getRegisterName(MCOperand_getReg(MCInst_getOperand(MI, 0)),
                                     AArch64_NoRegAltName));
      ...
      if (MI->csh->detail) {
        ...
        MI->flat_insn->detail->arm64
            .operands[MI->flat_insn->detail->arm64.op_count]
            .reg = MCOperand_getReg(MCInst_getOperand(MI, 0));
        MI->flat_insn->detail->arm64.op_count++;
        ...
        MI->flat_insn->detail->arm64
            .operands[MI->flat_insn->detail->arm64.op_count]
            .imm = SignExtend64(Value, RegWidth);
        MI->flat_insn->detail->arm64.op_count++;
      }
...
static bool printSysAlias(MCInst *MI, SStream *O) {
  ...
  if (NeedsReg) {
    SStream_concat(O, ", %s",
                   getRegisterName(MCOperand_getReg(MCInst_getOperand(MI, 4)),
                                   AArch64_NoRegAltName));
  }
  ...
  if (MI->csh->detail) {
    ...
    if (NeedsReg) {
      ...
      MI->flat_insn->detail->arm64
          .operands[MI->flat_insn->detail->arm64.op_count]
          .reg = MCOperand_getReg(MCInst_getOperand(MI, 4));
      MI->flat_insn->detail->arm64.op_count++;
    }
  }
...
static void printOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MCOperand_isReg(Op)) {
    ...
    if (MI->csh->detail) {
      if (MI->csh->doing_mem) {
        if (MI->flat_insn->detail->arm64
                .operands[MI->flat_insn->detail->arm64.op_count]
                .mem.base == ARM64_REG_INVALID) {
          MI->flat_insn->detail->arm64
              .operands[MI->flat_insn->detail->arm64.op_count]
              .mem.base = Reg;
        } else if (MI->flat_insn->detail->arm64
                       .operands[MI->flat_insn->detail->arm64.op_count]
                       .mem.index == ARM64_REG_INVALID) {
          MI->flat_insn->detail->arm64
              .operands[MI->flat_insn->detail->arm64.op_count]
...
static void printSysCROperand(MCInst *MI, unsigned OpNum, SStream *O) {
  MCOperand *Op = MCInst_getOperand(MI, OpNum);
  // assert(Op.isImm() && "System instruction C[nm] operands must be
  // immediates!");
...
static void printAddSubImm(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MCOperand_isImm(MO)) {
    ...
    unsigned Shift = AArch64_AM_getShiftValue(
        (int)MCOperand_getImm(MCInst_getOperand(MI, OpNum + 1)));
...
static void printShifter(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MI->csh->detail) {
    ...
    MI->flat_insn->detail->arm64
        .operands[MI->flat_insn->detail->arm64.op_count - 1]
        .shift.type = shifter;
    MI->flat_insn->detail->arm64
        .operands[MI->flat_insn->detail->arm64.op_count - 1]
        .shift.value = AArch64_AM_getShiftValue(Val);
  }
}

static void printShiftedRegister(MCInst *MI, unsigned OpNum, SStream *O) {
  SStream_concat0(
      O, getRegisterName(MCOperand_getReg(MCInst_getOperand(MI, OpNum)),
                         AArch64_NoRegAltName));
...
static void printMemExtend(MCInst *MI, unsigned OpNum, SStream *O,
                           char SrcRegKind, unsigned Width) {
  unsigned SignExtend =
      (unsigned)MCOperand_getImm(MCInst_getOperand(MI, OpNum));
  unsigned DoShift =
      (unsigned)MCOperand_getImm(MCInst_getOperand(MI, OpNum + 1));
...
static void printFPImmOperand(MCInst *MI, unsigned OpNum, SStream *O) {
  MCOperand *MO = MCInst_getOperand(MI, OpNum);
  float FPImm = MCOperand_isFPImm(MO)
                    ? MCOperand_getFPImm(MO)
                    : AArch64_AM_getFPImmFloat((int)MCOperand_getImm(MO));
...
static unsigned getNextVectorRegister(unsigned Reg, unsigned Stride) {
  while (Stride--) {
    if (Reg >= AArch64_Q0 && Reg <= AArch64_Q30) // AArch64_Q0 .. AArch64_Q30
      ...
      Reg = AArch64_Q0;
    else if (Reg >= AArch64_Z0 &&
             Reg <= AArch64_Z30) // AArch64_Z0 .. AArch64_Z30
      Reg += 1;
...
static void printGPRSeqPairsClassOperand(MCInst *MI, unsigned OpNum, SStream *O,
  ...
  if (MI->csh->detail) {
    ...
    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count]
        .type = ARM64_OP_REG;
    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count]
        .reg = AArch64_map_vregister(Even);
    MI->flat_insn->detail->arm64.op_count++;

    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count]
        .type = ARM64_OP_REG;
    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count]
        .reg = AArch64_map_vregister(Odd);
    MI->flat_insn->detail->arm64.op_count++;
  }
}
...
static void printVectorList(MCInst *MI, unsigned OpNum, SStream *O,
                            char *LayoutSuffix, MCRegisterInfo *MRI,
                            arm64_vas vas) {
#define GETREGCLASS_CONTAIN0(_class, _reg)                                     \
  MCRegisterClass_contains(MCRegisterInfo_getRegClass(MRI, _class), _reg)
  unsigned Reg = MCOperand_getReg(MCInst_getOperand(MI, OpNum));
  ...
  if (GETREGCLASS_CONTAIN0(AArch64_FPR64RegClassID, Reg)) {
    const MCRegisterClass *FPR128RC =
        MCRegisterInfo_getRegClass(MRI, AArch64_FPR128RegClassID);
  ...
  for (i = 0; i < NumRegs; ++i, Reg = getNextVectorRegister(Reg, 1)) {
    if (GETREGCLASS_CONTAIN0(AArch64_ZPRRegClassID, Reg))
      ...
      SStream_concat(O, "%s%s", getRegisterName(Reg, AArch64_vreg),
                     LayoutSuffix);
...
static void printVectorIndex(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MI->csh->detail) {
    MI->flat_insn->detail->arm64
        .operands[MI->flat_insn->detail->arm64.op_count - 1]
        .vector_index = (int)MCOperand_getImm(MCInst_getOperand(MI, OpNum));
  }
}
...
static void printImmSVE16(int16_t Val, SStream *O) { printUInt32Bang(O, Val); }

static void printImmSVE32(int32_t Val, SStream *O) { printUInt32Bang(O, Val); }

static void printImmSVE64(int64_t Val, SStream *O) { printUInt64Bang(O, Val); }
...
static void printZPRasFPR(MCInst *MI, unsigned OpNum, SStream *O, int Width) {
  ...
  SStream_concat0(
      O, getRegisterName(Reg - AArch64_Z0 + Base, AArch64_NoRegAltName));
}
...
static void printGPR64as32(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  SStream_concat0(O,
                  getRegisterName(getWRegFromXReg(Reg), AArch64_NoRegAltName));
}
...
static void printBarriernXSOption(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  //  TODO lookup Gen
  SStream_concat(O, "#%d", Val);
}

static void printGPR64x8(MCInst *MI, unsigned OpNum, SStream *O) {
  unsigned Val = MCOperand_getReg(MCInst_getOperand(MI, OpNum));
  SStream_concat0(O, MCRegisterInfo_getSubReg(MI->MRI, Val, AArch64_x8sub_0));
}

static void printSImm(MCInst *MI, unsigned OpNum, SStream *O, int Size) {
  unsigned Imm = MCOperand_getImm(MCInst_getOperand(MI, OpNum));
  if (Size == 8)
    printInt32Bang(O, (signed char)Imm);
  else if (Size == 16)
    printInt32Bang(O, (signed short)Imm);
  else
    printInt64Bang(O, Imm);
}
...
static void printBTIHintOp(MCInst *MI, unsigned OpNum, SStream *O) {
  unsigned Imm = (MCOperand_getImm(MCInst_getOperand(MI, OpNum)) ^ 32) >> 1;
...
static void printMatrixTile(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  if (MCOperand_isReg(Op))
    SStream_concat0(
        O, getRegisterName(MCOperand_getReg(Op), AArch64_NoRegAltName));
}

static void printMatrixTileVector(MCInst *MI, unsigned OpNum, SStream *O,
                                  bool IsVertical) {
  const MCOperand *RegOp = MCInst_getOperand(MI, OpNum);
  //  assert(RegOp.isReg() && "Unexpected operand type!");
  const char *RegName =
      getRegisterName(MCOperand_getReg(RegOp), AArch64_NoRegAltName);

  // Insert the horizontal/vertical flag before the suffix.
  for (; *RegName != 0; RegName++) {
    if (*RegName == '.') {
      SStream_concat1(O, IsVertical ? 'v' : 'h');
      RegName++;
      break;
    }
    SStream_concat1(O, *RegName);
  }
  SStream_concat0(O, RegName);
}

static void printMatrixIndex(MCInst *MI, unsigned OpNum, SStream *O) {
  SStream_concat(O, "%u", MCOperand_getImm(MCInst_getOperand(MI, OpNum)));
}
...
static void printMatrix(MCInst *MI, unsigned OpNum, SStream *O, int EltSize) {
  ...
  SStream_concat0(
      O, getRegisterName(MCOperand_getReg(RegOp), AArch64_NoRegAltName));
  switch (EltSize) {
  case 0:
    break;
  case 8:
...
static void printMatrixTileList(MCInst *MI, unsigned OpNum, SStream *O) {
  ...
  for (unsigned I = 0; I < MaxRegs; ++I) {
    ...
    if (Reg == 0)
      continue;
    SStream_concat0(
        O, getRegisterName(MatrixZADRegisterTable[I], AArch64_NoRegAltName));
...
static void printSVCROp(const MCInst *MI, unsigned OpNum, SStream *O) {
  const MCOperand *MO = MCInst_getOperand(MI, OpNum);
  unsigned svcrop = MCOperand_getImm(MO);
  for (int i = 0; i < ARR_SIZE(SVCRsList); i++) {
    if (SVCRsList[i].Index == svcrop) {
      SStream_concat0(O, SVCRsList[i].Name);
      return;
    }
  }
  assert(1 && "Unexpected SVCR operand!");
}
