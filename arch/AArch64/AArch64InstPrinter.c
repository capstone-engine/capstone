...
static bool isTblTbxInstruction(unsigned Opcode, const char **Layout,
				bool *IsTbx)
{
	switch (Opcode) {
...
{
	for (int i = 0; i < (sizeof(LdStNInstInfo) / sizeof(LdStNInstInfo[0]));
	     ++i) {
...
void AArch64AppleInstPrinter_printInst(MCInst *MI, uint64_t Address,
				       const char *Annot, SStream *O)
{
	unsigned Opcode = MCInst_getOpcode(MI);
	const char *Layout;
	...
	if (isTblTbxInstruction(MCInst_getOpcode(MI), &Layout, &IsTbx)) {
		...
		printRegNameAlt(O,
				MCOperand_getReg(
					MCInst_getOperand(MI, (ListOpNum + 1))),
				AArch64_vreg);
		;
		return;
	}
