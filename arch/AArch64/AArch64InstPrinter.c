        ...
        if (MCOperand_isImm(Op2) && MCOperand_getImm(Op2) == 0 && MCOperand_isImm(Op3)) {
            ...
            if (AsmMnemonic) {
                ...
                if (MI->csh->detail) {
                    ...
                    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].reg = MCOperand_getReg(
                            Op0);
                    MI->flat_insn->detail->arm64.op_count++;
#ifndef CAPSTONE_DIET
                    access = get_op_access(MI->csh, MCInst_getOpcode(MI), MI->ac_idx);
...
#endif
                    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].type = ARM64_OP_REG;
                    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].reg = getWRegFromXReg(
...
#endif
                MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].type = ARM64_OP_IMM;
                MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].imm =
                        (Is64Bit ? 64 : 32) - (int) MCOperand_getImm(Op2);
                MI->flat_insn->detail->arm64.op_count++;
#ifndef CAPSTONE_DIET
                access = get_op_access(MI->csh, MCInst_getOpcode(MI), MI->ac_idx);
                MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].access = access;
    ...
    if ((Opcode == AArch64_MOVZXi || Opcode == AArch64_MOVZWi) &&
        ...
        if (isMOVZMovAlias(Value, Shift,
            ...
            SStream_concat(O, "mov\t%s, ",
                           getRegisterName(MCOperand_getReg(MCInst_getOperand(MI, 0)), AArch64_NoRegAltName));
            ...
            if (MI->csh->detail) {
                ...
                MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].type = ARM64_OP_IMM;
                MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].imm = SignExtend64(Value,
                                                                                                                RegWidth);
                MI->flat_insn->detail->arm64.op_count++;
            }
...
static void printOperand(MCInst *MI, unsigned OpNum, SStream *O) {
    ...
    if (MCOperand_isReg(Op)) {
        ...
        if (MI->csh->detail) {
            if (MI->csh->doing_mem) {
                if (MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].mem.base ==
                    ARM64_REG_INVALID) {
                    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].mem.base = Reg;
                } else if (MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].mem.index ==
                           ARM64_REG_INVALID) {
...
static void printShifter(MCInst *MI, unsigned OpNum, SStream *O) {
    ...
    if (MI->csh->detail) {
        ...
        MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count -
                                              1].shift.value = AArch64_AM_getShiftValue(Val);
    }
}
    ...
    if (ExtType == AArch64_AM_UXTW || ExtType == AArch64_AM_UXTX) {
        ...
        if (((Dest == AArch64_SP || Src1 == AArch64_SP) &&
            ...
            if (ShiftVal != 0) {
                ...
                if (MI->csh->detail) {
                    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count -
                                                          1].shift.type = ARM64_SFT_LSL;
                    MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count -
                                                          1].shift.value = ShiftVal;
                }
            }
...
static void printMemExtendImpl(MCInst *MI, bool SignExtend, bool DoShift, unsigned Width,
    ...
    if (DoShift || IsLSL) {
        ...
        if (MI->csh->detail) {
            MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].shift.type = ARM64_SFT_LSL;
            MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].shift.value = Log2_32(
                    Width / 8);
        }
    }
}
...
#define GETREGCLASS_CONTAIN0(_class, _reg) MCRegisterClass_contains(MCRegisterInfo_getRegClass(MRI, _class), _reg)
    ...
    for (i = 0; i < NumRegs; ++i, Reg = getNextVectorRegister(Reg, 1)) {
        ...
        if (MI->csh->detail) {
            ...
            MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].type = ARM64_OP_REG;
            MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count].reg = AArch64_map_vregister(
                    Reg);
...
static void printVectorIndex(MCInst *MI, unsigned OpNum, SStream *O) {
    ...
    if (MI->csh->detail) {
        MI->flat_insn->detail->arm64.operands[MI->flat_insn->detail->arm64.op_count -
                                              1].vector_index = (int) MCOperand_getImm(MCInst_getOperand(MI, OpNum));
    }
}
