...
AArch64_AM_getExtendType(unsigned Imm) {
...
AArch64_AM_isSVEMoveMaskPreferredLogicalImmediate(int64_t Imm) {
  ...
  if (AArch64_AM_isSVEMaskOfIdenticalElements32(Imm) && isSVECpyImm32(Vec.S[0]))
    return false;

  if (AArch64_AM_isSVEMaskOfIdenticalElements16(Imm) && isSVECpyImm16(Vec.H[0]))
    return false;

  if (AArch64_AM_isSVEMaskOfIdenticalElements8(Imm) && isSVECpyImm8(Vec.B[0]))
    return false;
