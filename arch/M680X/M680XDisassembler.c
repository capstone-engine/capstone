...
{
  uuuu, // No register accessed (unchanged)
  rrrr, // All registers have (r)ead access
  wwww, // All registers have (w)rite access
  rwww, // First register has (r)ead, all others (w)rite access
  rrrw, // Last register has (w)rite, all others (r)ead access
  rmmm, // First register has (r)ead, all others (m)odified (read | write) acc.
  wrrr, // First register has (w)rite, all others (r)ead access
  mrrr, // First register is (m)odified (read | write), all others (r)ead
  mwww, // First register is (m)modified (read | write), all others (w)rite
  mmmm, // All register are (m)odified (read | write)
  mwrr, // First register is (m)odified, second op is (w)rite,
  // all others (r)ead access
} e_access_mode;
...
// M6800/1/3 instructions
// http://www.electronics.dit.ie/staff/tscarff/6800/Instructions/instructions.htm
// http://eecs.vanderbilt.edu/Courses/ee218/Asman.html
...
// Additional instructions only supported on M6801
...
// Additional instructions only supported on HD6301/HD6301
...
//
// M680X PAGE2 instructions (with prefix 0x10)
...
void M680X_get_insn_id(cs_struct *handle, cs_insn *insn, unsigned int id)
{
  ...
  {
    if (cpu->pageX_table_size[i] == 0 || (cpu->inst_pageX_table[i] == NULL))
    {
      break;
    }
    ...
    if (cpu->pageX_prefix[i] == pageX_prefix)
    {
      ...
      insn->id = (index >= 0) ?
         cpu->inst_pageX_table[i][index].insn : M680X_INS_ILLGL;
      return;
    }
  }
  ...
  for (i = 0; i < ARR_SIZE(cpu->overlay_table_size); ++i)
  {
    ...
    if ((index = binary_search(cpu->inst_overlay_table[i],
                               cpu->overlay_table_size[i], id & 0xff)) >= 0)
    {
...
static void add_reg_to_rw_list(MCInst *MI, m680x_reg reg, access_type access)
{
  ...
  {
    case MODIFY:
      if (!exists_reg_list(detail->regs_read, detail->regs_read_count, reg))
      {
        detail->regs_read[detail->regs_read_count++] = (uint16_t)reg;
      }

    // intentionally fall through

    case WRITE:
      if (!exists_reg_list(detail->regs_write, detail->regs_write_count, reg))
      {
        detail->regs_write[detail->regs_write_count++] = (uint16_t)reg;
      }
...
{
  {
    UNCHANGED, READ, WRITE, READ,  READ, READ,   WRITE, MODIFY, MODIFY,
    MODIFY, MODIFY
  },
  {
    UNCHANGED, READ, WRITE, WRITE, READ, MODIFY, READ,  READ,   WRITE,
    MODIFY, WRITE
  },
  {
    UNCHANGED, READ, WRITE, WRITE, READ, MODIFY, READ,  READ,   WRITE,
  ...
  {
    UNCHANGED, READ, WRITE, WRITE, WRITE, MODIFY, READ, READ,   WRITE,
    MODIFY, READ
  },
};
                                                     ...
                                                     m680x_info *info)
{
  static const insn_to_changed_regs changed_regs[] =
  {
    ...
    { M680X_INS_PSHX, { M680X_REG_S, M680X_REG_X, M680X_REG_INVALID }, },
    {
      M680X_INS_RTI, {
        ...
        M680X_REG_X, M680X_REG_PC, M680X_REG_INVALID
      },
    },
    ...
    {
      M680X_INS_SWI, {
        M680X_REG_S, M680X_REG_PC, M680X_REG_X, M680X_REG_A, M680X_REG_B,
        M680X_REG_CC, M680X_REG_INVALID
      },
    },
  ...
  if (MI->flat_insn->detail != NULL)
  {
    for (i = 0; i < ARR_SIZE(changed_regs); ++i)
    {
      ...
      {
        e_access_mode reg_access_mode = g_insn_props[m680x->insn].access_mode;

        for (j = 0; changed_regs[i].regs[j] != M680X_REG_INVALID; ++j)
        {
                                                     ...
                                                     m680x_info *info)
{
  static const insn_to_changed_regs changed_regs[] =
  {
    ...
    {
      M680X_INS_RTI, {
        M680X_REG_S, M680X_REG_CC, M680X_REG_A, M680X_REG_B,
    ...
    { M680X_INS_SEXW, { M680X_REG_D, M680X_REG_W, M680X_REG_INVALID }, },
    {
      M680X_INS_SWI, {
        M680X_REG_S, M680X_REG_PC, M680X_REG_U, M680X_REG_Y,
        M680X_REG_X, M680X_REG_DP, M680X_REG_B, M680X_REG_A,
        M680X_REG_CC, M680X_REG_INVALID
      },
    },
    {
                                    ...
                                    insn_desc *insn_description)
{
  ...
  switch (insn_description->handler_id)
  {
    ...
    case m6809_indexed_hdlr_id:
      ...
      return is_indexed_post_byte_valid(info, address, ir, insn_description);
    ...
    default:
      fprintf(stderr, "Internal error: Unexpected instruction handler id %d\n",
...
static void reg_bits_hdlr(MCInst *MI, m680x_info *info, uint16_t *address)
{
  ...
  switch (op0->reg)
  {
    ...
    default:
      fprintf(stderr, "Internal error: Unexpected operand0 register %d\n",
              op0->reg);
  }

  if ((m680x->insn == M680X_INS_PULU || (m680x->insn == M680X_INS_PULS)) &&
      ((reg_bits & 0x80) != 0))
  {
  ...
  for (bit_index = 0; bit_index < 8; ++bit_index)
  {
    ...
    {
      m680x->operands[m680x->op_count++].reg = reg_to_reg_ids[bit_index];
    }
  }
}
...
static void m6809_indexed_hdlr(MCInst *MI, m680x_info *info, uint16_t *address)
{
  ...
  if (((m680x->insn == M680X_INS_LEAU) || (m680x->insn == M680X_INS_LEAS) ||
       (m680x->insn == M680X_INS_LEAX) || (m680x->insn == M680X_INS_LEAY)) &&
...
static void immediate_hdlr(MCInst *MI, m680x_info *info, uint16_t *address)
{
  ...
  switch (imm_size)
  {
    ...
    default:
      op->imm = 0;
      fprintf(stderr, "Internal error: Unexpected immediate byte size %d.\n",
              imm_size);
  }
                                      ...
                                      uint16_t address)
{
  ...
  if (decode_insn(info, address, &insn_description))
  {
    ...
    if (insn_description.reg0 != M680X_REG_INVALID)
    {
      ...
      // First operand is a register which is part of the mnemonic
    ...
    (g_inst_handler[insn_description.handler_id])(MI, info, &address);
                          ...
                          MCInst *MI, uint16_t *size, uint64_t address, void *inst_info)
{
  ...
  if (m680x_setup_internals(info, cpu_type, (uint16_t)address, code, code_len))
  {
