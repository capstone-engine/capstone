...
{
	unsigned Op1, Op2;
	DecodeStatus S = Decode2OpInstruction(
		fieldFromInstruction_4(Insn, 0, 16), &Op1, &Op2);
	if (S != MCDisassembler_Success)
					 ...
					 uint64_t Address, const void *Decoder)
{
	...
	DecodeStatus S = Decode3OpInstruction(
		fieldFromInstruction_4(Insn, 0, 16), &Op1, &Op2, &Op3);
	if (S == MCDisassembler_Success) {
		DecodeGRRegsRegisterClass(Inst, Op1, Address, Decoder);
		DecodeGRRegsRegisterClass(Inst, Op2, Address, Decoder);
		DecodeGRRegsRegisterClass(Inst, Op3, Address, Decoder);
					 ...
					 uint64_t Address, const void *Decoder)
{
	...
	S = Decode3OpInstruction(fieldFromInstruction_4(Insn, 16, 16), &Op4,
				 &Op5, &Op6);
	if (S != MCDisassembler_Success)
		return S;
					 ...
					 uint64_t Address, const void *Decoder)
{
	...
	S = Decode2OpInstruction(fieldFromInstruction_4(Insn, 16, 16), &Op4,
				 &Op5);
	if (S != MCDisassembler_Success)
		return DecodeL5RInstructionFail(Inst, Insn, Address, Decoder);
			  ...
			  void *info)
{
	...
	if (MI->flat_insn->detail) {
		memset(MI->flat_insn->detail, 0,
		       offsetof(cs_detail, xcore) + sizeof(cs_xcore));
	}
...
void XCore_init(MCRegisterInfo *MRI)
{
	...
	*/
	...
	MCRegisterInfo_InitMCRegisterInfo(MRI, XCoreRegDesc, 17, 0, 0,
					  XCoreMCRegisterClasses, 2, 0, 0,
					  XCoreRegDiffLists, 0,
					  XCoreSubRegIdxLists, 1, 0);
}
