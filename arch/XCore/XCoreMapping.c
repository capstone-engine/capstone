	...
	if (i != 0) {
		...
		if (h->detail) {
			...
			if (insns[i].branch || insns[i].branch_indirect) {
				...
				if (insns[i].branch) {
					insn->detail->groups[insn->detail->groups_count] = XCORE_GRP_BRANCH;
					insn->detail->groups_count++;
				}
				// indirect branch group
				if (insns[i].branch_indirect) {
					insn->detail->groups[insn->detail->groups_count] = XCORE_GRP_BRANCH_INDIRECT;
					insn->detail->groups_count++;
				}
			}
#endif
...
static name_map group_name_maps[] = {
	...
	{ XCORE_GRP_JUMP, "jump" },
	{ XCORE_GRP_BRANCH, "branch" },
	{ XCORE_GRP_BRANCH_RELATIVE, "branch_relative" },
	{ XCORE_GRP_BRANCH_INDIRECT, "branch_indirect" },
};
#endif
