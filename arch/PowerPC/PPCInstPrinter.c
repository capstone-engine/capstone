...
void printS34ImmOperand(const MCInst *MI, unsigned OpNo, SStream *O);
...
static void set_mem_access(MCInst *MI, bool status) {
  ...
  if (status) {
    MI->flat_insn->detail->ppc.operands[MI->flat_insn->detail->ppc.op_count]
        .type = PPC_OP_MEM;
    MI->flat_insn->detail->ppc.operands[MI->flat_insn->detail->ppc.op_count]
...
static int getBICRCond(int bi) { return (bi - PPC_CR0EQ) >> 3; }

static int getBICR(int bi) { return ((bi - PPC_CR0EQ) & 7) + PPC_CR0; }
...
static char *printAliasBcc(MCInst *MI, SStream *OS, void *info) {
#define GETREGCLASS_CONTAIN(_class, _reg)                                      \
  MCRegisterClass_contains(MCRegisterInfo_getRegClass(MRI, _class),            \
                           MCOperand_getReg(MCInst_getOperand(MI, _reg)))
  SStream ss;
  const char *opCode;
  ...
  if (MCOperand_isReg(MCInst_getOperand(MI, 1)) &&
    ...
    if (decCtr) {
      ...
      if (cr > PPC_CR0) {
        if (MI->csh->detail) {
          MI->flat_insn->detail->ppc
              .operands[MI->flat_insn->detail->ppc.op_count]
              .type = PPC_OP_REG;
          MI->flat_insn->detail->ppc
              .operands[MI->flat_insn->detail->ppc.op_count]
              .reg = MCOperand_getReg(MCInst_getOperand(MI, 1));
          MI->flat_insn->detail->ppc.op_count++;
        }
      }
    } else {
...
void PPC_printInst(MCInst *MI, SStream *O, void *Info) {
  ...
  if (opcode == PPC_RLWINM) {
    unsigned char SH =
        (unsigned char)MCOperand_getImm(MCInst_getOperand(MI, 2));
    unsigned char MB =
        (unsigned char)MCOperand_getImm(MCInst_getOperand(MI, 3));
    unsigned char ME =
        (unsigned char)MCOperand_getImm(MCInst_getOperand(MI, 4));
  ...
  if (opcode == PPC_RLDICR || opcode == PPC_RLDICR_32) {
    unsigned char SH =
        (unsigned char)MCOperand_getImm(MCInst_getOperand(MI, 2));
    unsigned char ME =
        (unsigned char)MCOperand_getImm(MCInst_getOperand(MI, 3));
  ...
  if (opcode == PPC_DCBT || opcode == PPC_DCBTST) {
    unsigned char TH =
        (unsigned char)MCOperand_getImm(MCInst_getOperand(MI, 0));
  ...
  if (mnem != NULL) {
    if (strlen(mnem) > 0) {
      ...
      if (mnem[strlen(mnem) - 1] == '-' || mnem[strlen(mnem) - 1] == '+' ||
          mnem[strlen(mnem) - 1] == '.')
...
static void printPredicateOperand(MCInst *MI, unsigned OpNo, SStream *O,
  ...
  //		"Need to specify 'cc', 'pm' or 'reg' as predicate op
  // modifier!");
...
static void printU1ImmOperand(MCInst *MI, unsigned OpNo, SStream *O) {
  unsigned int Value =
      (unsigned int)MCOperand_getImm(MCInst_getOperand(MI, OpNo));
...
static void printU12ImmOperand(MCInst *MI, unsigned OpNo, SStream *O) {
  unsigned short Value =
      (unsigned short)MCOperand_getImm(MCInst_getOperand(MI, OpNo));
...
void printMemRegImmHash(MCInst *MI, unsigned OpNo, SStream *O) {
  SStream_concat(O, "%u", MCOperand_getImm(MCInst_getOperand(MI, OpNo)));
  SStream_concat(O, "(");
  printOperand(MI, OpNo + 1, O);
  SStream_concat(O, ")");
}
...
void printS34ImmOperand(const MCInst *MI, unsigned OpNo, SStream *O) {
  if (MCOperand_getImm(MCInst_getOperand(MI, OpNo))) {
