...
static void printAddress(MCInst *MI, unsigned Base, int64_t Disp,
                         unsigned Index, SStream *O) {
  printInt64(O, Disp);
  ...
  if (Base) {
    ...
    if (MI->csh->detail) {
      MI->flat_insn->detail->sysz.operands[MI->flat_insn->detail->sysz.op_count]
          .type = SYSZ_OP_MEM;
      MI->flat_insn->detail->sysz.operands[MI->flat_insn->detail->sysz.op_count]
          .mem.base = (uint8_t)SystemZ_map_register(Base);
      MI->flat_insn->detail->sysz.operands[MI->flat_insn->detail->sysz.op_count]
          .mem.index = (uint8_t)SystemZ_map_register(Index);
      MI->flat_insn->detail->sysz.operands[MI->flat_insn->detail->sysz.op_count]
          .mem.disp = Disp;
...
static void printBDLAddrOperand(MCInst *MI, int OpNum, SStream *O) {
  ...
  uint64_t Length =
      (uint64_t)MCOperand_getImm(MCInst_getOperand(MI, OpNum + 2));
...
static void printBDRAddrOperand(MCInst *MI, int OpNum, SStream *O) {
  ...
  if (MI->csh->detail) {
    ...
    MI->flat_insn->detail->sysz.operands[MI->flat_insn->detail->sysz.op_count]
        .mem.base = (uint8_t)SystemZ_map_register(Base);
    MI->flat_insn->detail->sysz.operands[MI->flat_insn->detail->sysz.op_count]
        .mem.length = (uint8_t)SystemZ_map_register(Length);
    MI->flat_insn->detail->sysz.operands[MI->flat_insn->detail->sysz.op_count]
        .mem.disp = (int64_t)Disp;
    MI->flat_insn->detail->sysz.op_count++;
  }
