...
static void printVMaskReg(MCInst *MI, unsigned OpNo, SStream *O);
static void printAtomicMemOp(MCInst *MI, unsigned OpNo, SStream *O);
static void printVTypeI(MCInst *MI, unsigned OpNo, SStream *O);
...
void RISCV_printInst(MCInst *MI, SStream *O, void *info) {
  ...
  if (MI->csh->detail && MI->flat_insn->detail->riscv.need_effective_addr)
...
static void printOperand(MCInst *MI, unsigned OpNo, SStream *O) {
  ...
  if (MCOperand_isReg(MO)) {
    ...
    printRegName(O, reg);
    if (MI->csh->detail) {
      MI->flat_insn->detail->riscv
          .operands[MI->flat_insn->detail->riscv.op_count]
          .type = RISCV_OP_REG;
      MI->flat_insn->detail->riscv
          .operands[MI->flat_insn->detail->riscv.op_count]
          .reg = reg;
...
static void printBranchOperand(MCInst *MI, unsigned OpNo, SStream *O) {
  ...
  //  if (PrintBranchImmAsAddress) { - just in it's simplest way
  ...
  SStream_concat(O, "%u", MCOperand_getImm(MO));
}
...
void printVMaskReg(MCInst *MI, unsigned OpNo, SStream *O) {
  ...
  if (MCOperand_getReg(MO) == 0) // No Register
    return;
  SStream_concat0(O, ", ");
...
void printAtomicMemOp(MCInst *MI, unsigned OpNo, SStream *O) {
  ...
  //  printAtomicMemOp can only print register operands
  SStream_concat0(O, "(");
  printRegName(O, MCOperand_getReg(MO));
  SStream_concat0(O, ")");
}
...
void printVType(unsigned VType, SStream *O) {
  unsigned VSEW = getVSEW(VType);
  unsigned VLMUL = getVLMUL(VType);

  unsigned Sew = 1 << (VSEW + 3);
  SStream_concat(O, "e%u", Sew);

  switch (VLMUL) {
  case RISCVVLMUL_LMUL_RESERVED:
    llvm_unreachable("Unexpected LMUL value!");
  case RISCVVLMUL_LMUL_1:
  case RISCVVLMUL_LMUL_2:
  case RISCVVLMUL_LMUL_4:
  case RISCVVLMUL_LMUL_8: {
    unsigned LMul = 1 << VLMUL;
    SStream_concat(O, ",m%u", LMul);
    break;
  }
  case RISCVVLMUL_LMUL_F2:
  case RISCVVLMUL_LMUL_F4:
  case RISCVVLMUL_LMUL_F8: {
    unsigned LMul = 1 << (8 - VLMUL);
    SStream_concat(O, ",mf%u", LMul);
    break;
  }
  }

  if (isTailAgnostic(VType))
    SStream_concat0(O, ",ta");
  else
    SStream_concat0(O, ",tu");

  if (isMaskAgnostic(VType))
    SStream_concat0(O, ",ma");
  else
    SStream_concat0(O, ",mu");
}
