...
void
RISCV_post_printer (csh ud, cs_insn * insn, char *insn_asm, MCInst * mci)
{
	/*
...
void
RISCV_printInst (MCInst * MI, SStream * O, void *info)
{
...
static void
printRegName (SStream * OS, unsigned RegNo)
{
...
static void
printOperand (MCInst * MI, unsigned OpNo, SStream * O)
{
	unsigned reg;
	...
	if (MCOperand_isReg (MO))
	  {
		  ...
		  if (MI->csh->detail)
		    {
			    MI->flat_insn->detail->riscv.operands[MI->
								  flat_insn->
								  detail->
								  riscv.
								  op_count].
...
static void
printFenceArg (MCInst * MI, unsigned OpNo, SStream * O)
{
	MCOperand *MO = MCInst_getOperand (MI, OpNo);
