...
static inline unsigned checkFeatureRequired(unsigned Bits, unsigned Feature,
  ...
  case RISCV_FeatureStdExtD:
    return Require;
  case RISCV_Feature64Bit: // In no case should we simplify this branch, since
                           // 'bool' here is a dummy macro
...
#define MASK_LS_INSN 0x0000007f
  ...
  if (0 == (opcode ^ 0x03) || 0 == (opcode ^ 0x07) || 0 == (opcode ^ 0x23) ||
      0 == (opcode ^ 0x27))
...
RISCVDisassembler_getInstruction(int mode, MCInst *MI, const uint8_t *code,
  ...
  if ((code[0] & 0x3) == 0x3) {
    ...
    *Size = 4;
    ...
    Inst =
        code[0] | (code[1] << 8) | (code[2] << 16) | ((uint32_t)code[3] << 24);
...
bool RISCV_getInstruction(csh ud, const uint8_t *code, size_t code_len,
  ...
  return MCDisassembler_Success == RISCVDisassembler_getInstruction(
                                       handle->mode, instr, code, code_len,
...
void RISCV_init(MCRegisterInfo *MRI) {
  ...
  */
  ...
  MCRegisterInfo_InitMCRegisterInfo(
      MRI, RISCVRegDesc, ARR_SIZE(RISCVRegDesc), 0, 0, RISCVMCRegisterClasses,
      ARR_SIZE(RISCVMCRegisterClasses), 0, 0, RISCVRegDiffLists, 0,
      RISCVSubRegIdxLists, ARR_SIZE(RISCVSubRegIdxLists), 0);
}
