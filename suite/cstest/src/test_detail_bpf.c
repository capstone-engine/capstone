		       ...
		       const TestDetailBPF *expected)
{
	...
	for (size_t i = 0; i < actual->op_count; ++i) {
		...
		case BPF_OP_IMM:
			...
			break;
		case BPF_OP_OFF:
			compare_uint32_ret(op->v.off, eop->off, false);
			break;
		case BPF_OP_MMEM:
			compare_uint32_ret(op->v.mmem, eop->mmem, false);
			break;
		case BPF_OP_MSH:
			compare_uint32_ret(op->v.msh, eop->msh, false);
			break;
		case BPF_OP_EXT:
			compare_enum_ret(op->v.ext, eop->ext, false);
			break;
		case BPF_OP_MEM:
			compare_reg_ret(*handle, op->v.mem.base, eop->mem_base,
					false);
			compare_uint32_ret(op->v.mem.disp, eop->mem_disp, false);
			break;
		}
	}
