...
TestDetailTMS320c64x *test_detail_tms320c64x_clone(TestDetailTMS320c64x *detail)
{
	...
	clone->funit_unit = detail->funit_unit ? strdup(detail->funit_unit) : NULL;
	...
	if (detail->operands_count > 0) {
		clone->operands = cs_mem_calloc(sizeof(TestDetailTMS320c64xOp*),
...
TestDetailTMS320c64xOp *test_detail_tms320c64x_op_clone(TestDetailTMS320c64xOp *op)
{
	...
	clone->mem_disptype = op->mem_disptype ? strdup(op->mem_disptype) : NULL;
	clone->mem_direction = op->mem_direction ? strdup(op->mem_direction) : NULL;
	clone->mem_modify = op->mem_modify ? strdup(op->mem_modify) : NULL;
	clone->mem_disp_const = op->mem_disp_const;
	clone->mem_disp_reg = op->mem_disp_reg ? strdup(op->mem_disp_reg) : NULL;
...
bool test_expected_tms320c64x(csh *handle, cs_tms320c64x *actual, TestDetailTMS320c64x *expected)
{
	...
	compare_reg_ret(*handle, actual->condition.reg, expected->cond_reg, false);
	...
	if (expected->funit_side_set) {
		compare_uint8_ret(actual->funit.side, expected->funit_side, false);
	}
	if (expected->funit_crosspath_set) {
		compare_uint8_ret(actual->funit.crosspath, expected->funit_crosspath, false);
	}
	...
	for (size_t i = 0; i < actual->op_count; ++i) {
		...
		case TMS320C64X_OP_REGPAIR:
			compare_reg_ret(*handle, op->reg + 1, eop->reg_pair_0, false);
			compare_reg_ret(*handle, op->reg, eop->reg_pair_1, false);
			break;
		...
		case TMS320C64X_OP_MEM:
			...
			compare_enum_ret(op->mem.direction, eop->mem_direction, false);
			compare_tbool_ret(op->mem.scaled, eop->mem_scaled, false);
			compare_enum_ret(op->mem.disptype, eop->mem_disptype, false);
			if (op->mem.disptype == TMS320C64X_MEM_DISP_REGISTER) {
				compare_reg_ret(*handle, op->mem.disp, eop->mem_disp_reg, false);
			} else {
				compare_uint_ret(op->mem.disp, eop->mem_disp_const, false);
			}
			compare_enum_ret(op->mem.modify, eop->mem_modify, false);
